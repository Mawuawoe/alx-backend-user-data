...
def register_user(email: str, password: str) -> None:
    ...
    assert payload == {"email": email, "message": "user created"}, f"Unexpected response: {payload}"
...
def log_in(email: str, password: str) -> str:
    ...
    payload = response.json()
    assert "session_id" in response.cookies, "No session_id in response cookies"
...
def update_password(email: str, reset_token: str, new_password: str) -> None:
    ...
    payload = response.json()
    assert payload == {"email": email, "message": "Password updated"}, f"Unexpected response: {payload}"
